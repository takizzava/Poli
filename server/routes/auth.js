...
r.post('/signup', async (req, res) => {
  try {
    ...
    if (msg.includes('UNIQUE') || msg.includes('unique')) return res.status(409).json({ error: 'email already registered' })
...
r.post('/logout', (_req, res) => { res.clearCookie('sid', cookieOpts()); res.json({ ok: true }) })
